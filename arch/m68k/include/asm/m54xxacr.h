...
/*
 * If we have a DMA global pool, we need to map it as non cacheable.
 * For overlapping regions, the lower ACR number takes priority. Use ACR1
 * for the DMA pool, and ACR4 for the rest of RAM.
 */
#if defined(CONFIG_DMA_GLOBAL_POOL)
#define ACR1_MODE	(ACR_BA(CONFIG_DMABASE)+ACR_ADMSK(CONFIG_DMASIZE)+ \
			 ACR_ENABLE+ACR_SUPER+ACR_CM_OFF_PRE+ACR_SP)
#if defined(CONFIG_CACHE_COPYBACK)
#define ACR4_MODE	(ACR_BA(CONFIG_RAMBASE)+ACR_ADMSK(CONFIG_RAMSIZE)+ \
