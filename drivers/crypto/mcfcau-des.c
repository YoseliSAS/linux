		      ...
		      unsigned int keylen)
{
	...
	*
	*/
	if (*flags & CRYPTO_TFM_REQ_FORBID_WEAK_KEYS) { /* FIPS 74 */
		if (key[0] < 0xE001E00l) {
			if (key[0] < 0x1FE01FE0) {
				if (key[0] < 0x01E001E0) {
					if (((key[0] == 0x01010101) &&
...
static void mcfcau_des_encrypt(struct crypto_tfm *tfm, u8 *dst, const u8 *src)
{
...
static void mcfcau_des_decrypt(struct crypto_tfm *tfm, u8 *dst, const u8 *src)
{
	struct mcfcau_des_ctx *ctx = crypto_tfm_ctx(tfm);
	u32 *des_key_tmp = ctx->expkey;
	unsigned long iflags;
