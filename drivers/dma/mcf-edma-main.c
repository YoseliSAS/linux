...
static irqreturn_t mcf_edma_err_handler(int irq, void *dev_id)
{
	...
	if (err) {
		handled = true;
		for_each_set_bit(bit, (unsigned long *)&err, 32) {
			ch = bit;
	...
	if (err) {
		handled = true;
		for_each_set_bit(bit, (unsigned long *)&err, 32) {
			ch = bit + 32;  /* Offset to channels 32-63 */
			fsl_edma_disable_request(&mcf_edma->chans[ch]);
			iowrite8(EDMA_CERR_CERR(ch), regs->cerr);
			fsl_edma_err_chan_handler(&mcf_edma->chans[ch]);
		}
				...
				struct fsl_edma_engine *mcf_edma)
{
	...
	for (ret = 0, i = res->start; i <= res->end; ++i) {
		char *irq_name;
		irq_name = devm_kasprintf(&pdev->dev, GFP_KERNEL, "eDMA-%d", i - res->start);
	...
	for (ret = 0, i = res->start; i <= res->end; ++i) {
		char *irq_name;
		irq_name = devm_kasprintf(&pdev->dev, GFP_KERNEL, "eDMA-%d", 16 + i - res->start);
		ret |= request_irq(i, mcf_edma_tx_handler, 0, irq_name, mcf_edma);
	}
	if (ret)
		return ret;
